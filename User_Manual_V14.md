...
## 49. ⚠️ Netlify自動部署目前故障中，push後要手動額外部署（2026-07-20）

### 現況
`bwbc-iad`這個Netlify網站，跟GitHub之間的自動部署連接壞掉了。**`git push`到GitHub本身完全正常**，但Netlify原本應該「偵測到GitHub有新commit就自動建置部署」這個機制失效，錯誤訊息是`Host key verification failed`（GitHub連接的SSH金鑰驗證失敗）。已經試過在Netlify後台「重新連接repository」，沒有解決。

### 這代表什麼
**只跑`git push`，網站不會更新。** 每次push完，還要額外手動跑一次部署指令，網站才會真的反映最新內容。
...
第一次使用電腦要先登入一次（之後應該不用重複登入）：
```powershell
npx netlify-cli login
```

### 注意事項
- 這只是**繞過**問題、不是修好問題本身——`Host key verification failed`這個根本原因還沒解決
- 根本解法（尚未執行）：聯絡Netlify客服處理連接問題，或考慮重建一個全新的Netlify專案取代這個壞掉的
...
*本手冊持續更新。每次系統功能變更時同步更新本文件。*
*系統版本：admin v10.1 / teacher v2.17 / student v3.1 / live-poll v1.0　最後更新：2026-08-08*
